...
```{r}
student_grades <- c(94,96,42,98,72,82)
...
```{r}
# Create a dataframe from three vectors, each vector is its own column.
...
my_array <- array(
  ...
  dim <- c(3,3,3),
  #row/col names
  dimnames <- list(c('row1','row2','row3'), c('col1','col2','col3'), c('matrix1','matrix2','matrix3'))
)
my_array
```
...
this_array <- array(
  ...
  dim <- c(3,3,2),
  dimnames <- list(c('one','two','three'),c('this','smells','funny'),c('Old','New'))
)
this_array
```
...
```{r}
# Second row, second column, first matrix
this_array[2,2,1]

# Second row, second column, second matrix
this_array[2,2,2]
```
